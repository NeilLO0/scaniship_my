...
class ScanishipModule(reactApplicationContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactApplicationContext) {
    private val EMIT_MIN_INTERVAL_MS = 200L
    private val MAX_TAGS_BUFFER = 500
...
//                    Toast.makeText(currentActivity?.baseContext, time.toString(), Toast.LENGTH_SHORT).show()
    ...
    private val inventoryThread = object : Runnable {
        override fun run() {
            try {
                ...
                val valuesSnapshot: List<TagInfo>
                val keysSnapshot: List<String>
                synchronized(tagLock) {
                    valuesSnapshot = ArrayList(tagInfoMap.values)
                    keysSnapshot = ArrayList(tagInfoMap.keys)
                    tagInfoList.clear()
                    tagInfoList.addAll(valuesSnapshot)
                    listEPC.clear()
                    listEPC.addAll(keysSnapshot)
                }

                    val now = System.currentTimeMillis()
                    if (now - lastEmitAt >= EMIT_MIN_INTERVAL_MS) {
                ...
                android.util.Log.e("ScanishipModule", "inventoryThread crashed", t)
    ...
    private fun pooled6cData(info: Reader.TAGINFO) {
        val epcBytes = info.EpcId ?: return
        if (epcBytes.isEmpty()) return
        val epcAndTid = Tools.Bytes2HexString(info.EpcId, info.EpcId.size)
        if (epcAndTid.isBlank() || epcAndTid.length !in 4..128 || !isHexString(epcAndTid)) return
        synchronized(tagLock) {
            if (!tagInfoMap.containsKey(epcAndTid) && tagInfoMap.size >= MAX_TAGS_BUFFER) {
                return
            }
            if (tagInfoMap.containsKey(epcAndTid)) {
                val tagInfo: TagInfo = tagInfoMap.getValue(epcAndTid)
