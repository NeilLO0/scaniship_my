...
export type QueuedBatch = {
  ...
  ownerUserId: number;
  chunkSize?: number;
  chunkStates?: BatchChunkState[];
};

export type BatchChunkState = {
  index: number;
  start: number;
  end: number;
...
async function readQueue(): Promise<QueuedBatch[]> {
  ...
  let parsedFromEncrypted = false;
  try {
    parsed = parse(decrypt(raw));
    if (parsed) {
      parsedFromEncrypted = true;
    } else {
      parsed = parse(raw);
    }
  ...
  const now = Date.now();
  let filtered = parsed.filter((item) => now - item.createdAt <= EXPIRY_MS);
  if (filtered.length > MAX_QUEUE_BATCHES) {
    filtered = filtered.slice(-MAX_QUEUE_BATCHES);
  }
  const needsRewrite = !parsedFromEncrypted || filtered.length !== parsed.length;
  if (needsRewrite) {
...
export async function enqueueBatch(batch: QueuedBatch) {
  ...
  if (queue.length >= MAX_QUEUE_BATCHES) {
    queue.splice(0, queue.length - (MAX_QUEUE_BATCHES - 1));
  }
  queue.push(batch);
  if (__DEV__) {
...
export async function updateBatch(batch: QueuedBatch) {
  const queue = await readQueue();
  const idx = queue.findIndex((item) => item.id === batch.id);
  if (idx >= 0) {
    queue[idx] = batch;
    await writeQueue(queue);
  }
