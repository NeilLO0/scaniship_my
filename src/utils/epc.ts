...
const PACKAGE_REGEX = /^[0-9A-Z]{10,11}$/;
...
export function convertEpcToPackage(epc: string): string {
  ...
  if (normalized.length < 24 || !HEX_REGEX.test(normalized)) {
    throw new Error('EPC 長度或格式錯誤，已視為無效標籤');
  }

  let hex = normalized;
  if (hex.startsWith('FFFF')) {
    // 新規則：跳過前 4 碼，取後續 20 碼轉 ASCII
    hex = hex.slice(4, 24);
    let ascii = hexToAscii(hex);
    ascii = ascii.replace(/\x00+$/g, '');
    const filtered = ascii.replace(/[^0-9A-Za-z]/g, '').toUpperCase();
    if (!PACKAGE_REGEX.test(filtered)) {
      throw new Error('EPC 轉碼內容不符合包裝編號規則，已視為無效標籤');
    }
    return filtered;
  }
